...
const registerSchema = Yup.object({
    username: Yup.string().required("Username is required").min(3, "Username must be at least 3 characters").max(20, "Username can't be more than 20 characters"),
                    ...
                    <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl shadow-xl shadow-slate-200/50 p-10">
                        ...
                        <form onSubmit={formik.handleSubmit} className="space-y-5">
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-12 pl-12 pr-12 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 text-sm font-medium transition-all duration-200 focus:outline-none focus:border-emerald-500 focus:bg-white focus:shadow-emerald-500/10"
                            ...
                            {formik.touched.username && formik.errors.username && (
                                <div className="rounded-lg bg-red-50 border border-red-200 p-3">
                                    <p className="text-xs text-red-600 font-medium text-center">{formik.errors.username}</p>
                                </div>
                            )}
                            <div className="space-y-2">
                            ...
                            {formik.touched.email && formik.errors.email && (
                                <div className="rounded-lg bg-red-50 border border-red-200 p-3">
                                    <p className="text-xs text-red-600 font-medium text-center">{formik.errors.email}</p>
                                </div>
                            )}
                            <div className="space-y-2">
                            ...
                            {formik.touched.password && formik.errors.password && (
                                <div className="rounded-lg bg-red-50 border border-red-200 p-3">
                                    <p className="text-xs text-red-600 font-medium text-center">{formik.errors.password}</p>
                                </div>
                            )}
                            {error && (
                                <div className="rounded-lg bg-red-50 border border-red-200 p-3">
                        ...
                        <div className="mt-8 pt-6 border-t border-slate-200/60">
                            <p className="text-center text-sm text-slate-600">
                                ...
                                <Link href='/login' className="font-semibold text-emerald-600 hover:text-emerald-700 transition-colors duration-200">
                                    Sign in
                                </Link>
                            </p>
                        </div>
